...
export const makeGitManager = Effect.fn("makeGitManager")(function* () {
  ...
  const preparePullRequestThread: GitManagerShape["preparePullRequestThread"] = Effect.fn(
    ...
    return yield* Effect.gen(function* () {
      ...
      const worktreeResult = yield* worktrunk
        .switchPR({
          cwd: input.cwd,
        ...
        .pipe(
          Effect.mapError((error) =>
            gitManagerError("preparePullRequestThread", error.message, error),
          ),
        );

      // Configure upstream tracking for the PR branch
      ...
      return {
        pullRequest,
        branch: details.branch ?? worktreeResult.branch,
        worktreePath,
      };
