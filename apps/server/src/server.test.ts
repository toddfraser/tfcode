...
import { Worktrunk, type WorktrunkShape } from "./worktrunk/Services/Worktrunk.ts";
...
const buildAppUnderTest = (options?: {
  ...
  layers?: {
    ...
    worktrunk?: Partial<WorktrunkShape>;
  };
}) =>
  Effect.gen(function* () {
    ...
    const appLayer = servedRoutesLayer.pipe(
      ...
      Layer.provide(
        Layer.mock(Worktrunk)({
          checkInstalled: () => Effect.succeed({ installed: true as const, version: "0.0.0-test" }),
          list: () => Effect.succeed([]),
